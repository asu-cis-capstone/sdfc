<%@ Page Language="C#" AutoEventWireup="true" Inherits="_Default" MasterPageFile="~/Desktop/Desktop.master" Codebehind="Default.aspx.cs" %>
<%@ MasterType VirtualPath="~/Desktop/Desktop.master" %>

<asp:Content ID="DefaultDesktopContent" runat="server" ContentPlaceHolderID="DesktopContentPlaceholder">
    <h2>Hello World, from the content page!</h2> <!--Test Content-->

        <p>default page</p>
        <div id="recentRecords">
            <div class="recentRecord">
                <asp:Label runat="server" ID="recordType" Text="hello">vhdgjfhjgukhil</asp:Label>
            </div>
        </div>

</asp:Content>